...
if [ ! -f "$base_dir/desktop/package/desktop-$version-jar-with-dependencies.jar" ]; then
    echo Building application
    $base_dir/mvnw.sh :desktop:clean :desktop:install
    jar_file=$base_dir/desktop/build/libs/desktop-$version-jar-with-dependencies.jar
    if [ ! -f "$jar_file" ]; then
    ...
    jar_file=$base_dir/desktop/package/desktop-$version-jar-with-dependencies.jar
...
chmod o+rx "$src_dir/desktop-$version-jar-with-dependencies.jar"

# Remove previously generated packages so we can later determine if they are actually generated successfully
if [ -f "$base_dir/desktop/package/linux/1m5-$version.deb" ]; then
    rm "$base_dir/desktop/package/linux/1m5-$version.deb"
fi
if [ -f "$base_dir/desktop/package/linux/1m5-$version.rpm" ]; then
    rm "$base_dir/desktop/package/linux/1m5-$version.rpm"
fi
...
$JAVA_HOME/bin/javapackager \
    ...
    -native deb \
    -name 1M5 \
    -title "A decentralized censorship-resistant networked service privacy platform." \
    -vendor 1M5 \
    -outdir $base_dir/desktop/package/linux \
    -srcdir $src_dir \
    -srcfiles desktop-$version-jar-with-dependencies.jar \
    -appclass io.onemfive.desktop.DesktopAppMain \
    -BjvmOptions=-Xss1280k \
    -outfile 1M5-$version \
    -v

if [ ! -f "$base_dir/desktop/package/linux/1m5-$version.deb" ]; then
    echo No deb file found at $base_dir/desktop/package/linux/1m5-$version.deb
    exit 3
fi
...
if [ ! -f "$base_dir/desktop/package/linux/1m5-$version-1.x86_64.rpm" ]; then
    echo No rpm file found at $base_dir/desktop/package/linux/1m5-$version-1.x86_64.rpm
    exit 3
fi
...
#     rm "$base_dir/desktop/package/linux/1m5-$version.deb"
# fi
mv $base_dir/desktop/package/linux/1m5-$version.deb $base_dir/desktop/package/linux/1m5-$version.deb

echo SHA256 of $base_dir/desktop/package/linux/1m5-$version.deb:
shasum -a256 $base_dir/desktop/package/linux/1m5-$version.deb | awk '{print $1}' | tee $base_dir/desktop/package/linux/1m5-$version.deb.txt

# FIXME: My Ubuntu somehow also deletes the lower case file
# if [ -f "$base_dir/desktop/package/linux/1m5-$version-1.x86_64.rpm" ]; then
#     rm "$base_dir/desktop/package/linux/1m5-$version-1.x86_64.rpm"
# fi
mv $base_dir/desktop/package/linux/1m5-$version-1.x86_64.rpm $base_dir/desktop/package/linux/1m5-$version.rpm

echo SHA256 of $base_dir/desktop/package/linux/1m5-$version.rpm:
shasum -a256 $base_dir/desktop/package/linux/1m5-$version.rpm | awk '{print $1}' | tee $base_dir/desktop/package/linux/1m5-$version.rpm.txt
